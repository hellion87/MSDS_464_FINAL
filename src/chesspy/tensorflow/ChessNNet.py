...
class ChessNNet:
    def __init__(self, game, args):
        ...
        with self.graph.as_default():
            self.input_boards = tf.compat.v1.placeholder(tf.float32, shape=[None, self.board_x, self.board_y, self.board_z])  # s: batch_size x board_x x board_y
            ...
            #                      [self.board_x, self.board_y, self.board_z, 1])  # batch_size  x board_x x board_y x 1
            ...
            s_fc1 = Dropout(
                Relu(BatchNormalization(Dense(h_conv4_flat, 1024, use_bias=False), axis=1, training=self.isTraining)),
                rate=self.dropout)  # batch_size x 1024
            s_fc2 = Dropout(
                Relu(BatchNormalization(Dense(s_fc1, 512, use_bias=False), axis=1, training=self.isTraining)),
    ...
    def conv2d(self, x, out_channels, padding):
        return tf.compat.v1.layers.conv2d(x, out_channels, kernel_size=[3, 3], padding=padding)

    def calculate_loss(self):
        self.target_pis = tf.compat.v1.placeholder(tf.float32, shape=[None, self.action_size])
        self.target_vs = tf.compat.v1.placeholder(tf.float32)
