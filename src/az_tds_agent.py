...
class AZTDSNode:
    def __init__(self, game_state: str, parent: AZTDSNode = None, c=1.0):
        self.game_state = game_state  # FEN string
        self.is_expanded = False  # False indicates that this is a leaf node
        self.parent = parent
        self.children: Dict[str, AZTDSNode] = {}  # Dict[move, AZTDSNode]
        self.total_value: int = 0  # sum of total rewards
        self.Q : float = 0 # expected game result from this position
        self.number_visits: int = 0
        self.c: float = c  # exploration hyper-parameter for AZTDS. Increase c to explore.

    def select_leaf(self) -> AZTDSNode:
        current = self
    ...
    def backup(self, value_estimate: int):
        ...
        while current.parent is not None:
            ...
            current.Q = current.total_value / current.number_visits
    ...
    def add_child(self, move: str):
        ...
        self.children[move] = AZTDSNode(child_state, parent=self, c=self.c)

    def best_child(self) -> AZTDSNode:
        best_node = None
        best_value = -np.inf
        for key in self.children:
            node = self.children[key]
            U =  (node.c * math.sqrt(math.log(node.parent.number_visits + 1) / (node.number_visits + 1)))
            Q = node.Q
            if Q + U > best_value
                best_value = Q + U
                best_node = node
...
class AZTDSAgent(Agent):
    """Upper Confidence Bounds for Trees (AZTDS) algorithm applied to chess.

    The AZTDS algorithm is a specific implementation of the more general Monte
    Carlo Tree Search (MCTS) algorithm. AZTDS uses the Upper-Confidence-Bound
    ...
    def observe(self, reward: int, observation: str) -> str:
        ...
        best_action = self._AZTDS_search(observation, num_iterations=self.iterations)
        return best_action

    def _AZTDS_search(self, game_state: str, num_iterations: int, return_probs=False):
        ...
        # Otherwise create a new tree from scratch
            ...
            root = AZTDSNode(game_state, c=self.c)
    ...
    def _bfs(self, root: AZTDSNode, fen: str):
    ...
    def _rollout(self, state: str) -> int:
        ...
        if board.is_game_over():
            ...
            return -1 * self.nnet(encoded_state)
